...
cur.execute("select distinct(s_roll) from student_table_1 where student_table_1.student_email in (select student_class.student_email from student_class where password_c=%s)",row[1])
row_1=cur.fetchall()
    ...
    if txt_option.get()=="":
        ...
        try:
            ...
            if txt_option.get()=="CT_1":
                    ...
                    k=0
                    for i in row_1:
                        cur.execute("insert into grade (password_c,s_roll,ct_1) values(%s,%s)",
                        (
                            row[1],
                            i[0],
                            a[k]
...
btn=Button(root,text="Enter",font=("times new roman",15,"bold"),fg="black",bg="#ECE5C7",command=enter_btn).place(x=610,y=170+(j*60),width=150)
